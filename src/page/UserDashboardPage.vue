<template>
  <div>
    ...
    <p class="text-3xl text-center font-semibold text-black">All Users</p>
    <router-link to="/admin/new_data">
      <button class="bg-lime-700 px-2 py-2 text-sm rounded-md text-white font-semibold my-2">Tambah Data</button>
    </router-link>
    ...
    <table class="border-2 border-lime-700  w-full mb-10">
      ...
      <tbody v-for="(item, index) in dataUser.data" :key="index" class="border-2 border-lime-700 relative mx-auto">
        <tr
          ...
          <td class="px-5 py-3 whitespace-nowrap">{{ item.username }}</td>
          <td class="px-5 py-3 whitespace-nowrap">{{ item.email }}</td>
          <td class="px-5 py-3 whitespace-nowrap">{{ item.telephone }}</td>
          <td>
...
//     userStore.setCatalog("jajanan_pasar")
//   })
// }
