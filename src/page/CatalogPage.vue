<template>
  <div class="md:w-[80%] mx-auto font-openSans mt-10">
    <div class="w-full md:relative p-5">
      <p class="text-2xl px-10 font-semibold">Kategori Produk</p>
      <div
        ...
        <div v-for="(item, index) in category" :key="index">
          <div
            class="flex justify-center items-center w-[100px] h-[100px] px-2 py-3 mr-3 rounded-lg cursor-pointer shadow-lg transition-all"
      ...
      <div class="w-full grid md:grid-cols-4 grid-cols-2 justify-center items-center md:gap-10 gap-5 my-5 px-3"
        ...
        <CatalogComponent v-for="(item, index) in catalog" :key="index" :item="item" />
        <CartNavigation />
      </div>
...
const CatalogComponent = defineAsyncComponent({
  loader: () => import('../components/CardComponent.vue' /* webpackChunkName: "productComponent" */),
})
